...
const ExamData = ({ errors, setValue, trigger, defaultExam }) => {
    ...
    const [examListModality, setExamListModality] = useState([]);
    ...
    const renderModalityList = (modalitiles, group, elem) => {
        return(
            ...
            >
                { modalitiles.map(modality => <FormControlLabel  value={modality.term} control={<Radio />} label={`${modality.term} (${modality.decription})`}
                                            ...
                                            item?.list?.map(elem => (
                                                <StyledTableRow key={elem._id}>
                                                    <StyledTableCell align="left">
                                                        ...
                                                        />} label={elem.name} />
                                                        {selectedExam.includes(elem._id) ? renderModalityList(elem.modality, { name: item.name, _id: item._id }, elem):null}
                                                    </StyledTableCell>
                                                </StyledTableRow>
                                            ))
                                            :
